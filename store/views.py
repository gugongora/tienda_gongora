...
def product_detail(request, product_id):
    response = requests.get(f'54.208.45.218/api/productos/{product_id}')
...
def dashboard_interno(request):
    ...
    try:
        suc_response = session.get("54.208.45.218/api/sucursales/", headers=headers)
    ...
    if request.method == 'POST':
        if 'consultar_stock' in request.POST:
            ...
            if sucursal_id:
                url = f'54.208.45.218/api/sucursales/{sucursal_id}/stock/'
                try:
            ...
            if pedido_id:
                url = f'54.208.45.218/api/pedidos/{pedido_id}/'
                try:
                    response = session.get(url, headers=headers)
                    if response.status_code == 200:
